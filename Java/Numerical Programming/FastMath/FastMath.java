...
public class FastMath {
	...
	public static Gleitpunktzahl invSqrt(Gleitpunktzahl x) {
        int ieeeInterpretation = gleitpunktzahlToIEEE(x);
        int res = ieeeInterpretation;
        ieeeInterpretation >>= 1;
        ieeeInterpretation += (res & 1);
        ieeeInterpretation = MAGIC_NUMBER - ieeeInterpretation;
		return iEEEToGleitpunktzahl(ieeeInterpretation);
	}
	...
	public static double absInvSqrtErr(Gleitpunktzahl x) {
		return Math.abs(invSqrtErr(x));
	}

	public static double invSqrtErr(Gleitpunktzahl x) {
		double exact = 1 / Math.sqrt(x.toDouble());
		double approx = invSqrt(x).toDouble();
		double err = exact - approx;
